...
def validate_result(result: dict) -> bool:
    ...
    required_fields = ['paper_id', 'answer_1', 'answer_2', 'answer_3', 'answer_4']
    return all(field in result for field in required_fields)


def run_verification(client, prompt_text: str, model: str = "deepseek/deepseek-v3.1-terminus", max_retries: int = 3):
    """Run verification through Novita API with retry logic."""
...
def process_conference(conference_dir: Path, client, output_dir: Path, resume: bool = True, start_paper_id: str = None, end_paper_id: str = None):
    """Process all verification prompts for a conference."""

    conference_name = conference_dir.name
    prompt_files = sorted(conference_dir.glob("*_verify_normalize.txt"))

    # Filter by paper ID range if specified
    if start_paper_id or end_paper_id:
        filtered_files = []
        for prompt_file in prompt_files:
            paper_id = prompt_file.stem.replace('_verify_normalize', '')

            # Extract numeric part from paper_id (e.g., "2023.acl-main.100" -> "100")
            # Handles formats like "2023.acl-main.100" or "2020.acl-main.1"
    ...
    for prompt_file in tqdm(prompt_files, desc=f"{conference_name}", leave=False):
        paper_id = prompt_file.stem.replace('_verify_normalize', '')
        output_file = conf_output_dir / f"{paper_id}_verified.json"
        ...
        # Run verification
        result, error = run_verification(client, prompt_text)
...
def main():
    """Main function to run verification on all prompts."""

    # Parse command-line arguments
    parser = argparse.ArgumentParser(description='Run verification on NLG evaluation papers')
    ...
    # Paths
    prompts_dir = Path('../verify_normalize_prompts')
    output_dir = Path('../llm-merged-results/novita')
    ...
    print("=" * 80)
    print("Running Verification with DeepSeek v3.1 via Novita AI")
    print("=" * 80)
    ...
    print(f"Average time per paper: {elapsed_time/max(total_completed-total_skipped, 1):.1f} seconds")
