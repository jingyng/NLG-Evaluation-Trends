#!/usr/bin/env python3
"""Aggregate summarization paper data from JSON files."""
...
def aggregate_summarization_papers(base_path):
    """Aggregate all summarization paper data."""
    ...
    print(f"Found {len(json_files)} summarization papers")
...
def save_aggregated_data(papers_data, metric_combinations, human_criteria_list, llm_judge_list, output_dir):
    ...
    papers_csv = output_dir / 'summarization_papers_aggregated.csv'
    ...
    combos_csv = output_dir / 'summarization_metric_combinations.csv'
    ...
    if human_criteria_list:
        ...
        human_csv = output_dir / 'summarization_human_criteria.csv'
    ...
    if llm_judge_list:
        ...
        llm_csv = output_dir / 'summarization_llm_judges.csv'
        llm_df.to_csv(llm_csv, index=False)
        print(f"Saved LLM judge data to: {llm_csv}")

    # Print statistics
    print("\n" + "="*60)
    print("SUMMARIZATION PAPERS STATISTICS")
    print("="*60)
...
def main():
    ...
    output_dir = script_dir / 'summarization_analysis_data'

    print(f"Aggregating summarization papers from: {base_path}\n")

    papers_data, metric_combinations, human_criteria_list, llm_judge_list = aggregate_summarization_papers(base_path)
