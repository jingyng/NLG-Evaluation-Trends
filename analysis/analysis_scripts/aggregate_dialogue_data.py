#!/usr/bin/env python3
"""Aggregate dialogue generation paper data from JSON files."""
...
def aggregate_dialogue_papers(base_path):
    """Aggregate dialogue paper data: papers whose only task is Dialogue Generation."""
    ...
    print(f"Found {len(json_files)} candidate papers under {base_path}")

    for json_file in json_files:
        try:
            info = extract_paper_info(json_file)
            # Filter: keep papers whose only task is the target task.
...
def save_aggregated_data(papers_data, metric_combinations, human_criteria_list, llm_judge_list, output_dir):
    ...
    papers_csv = output_dir / 'dialogue_papers_aggregated.csv'
    ...
    combos_csv = output_dir / 'dialogue_metric_combinations.csv'
    ...
    if human_criteria_list:
        ...
        human_csv = output_dir / 'dialogue_human_criteria.csv'
    ...
    if llm_judge_list:
        ...
        llm_csv = output_dir / 'dialogue_llm_judges.csv'
        llm_df.to_csv(llm_csv, index=False)
        print(f"Saved LLM judge data to: {llm_csv}")

    # Print statistics
    print("\n" + "="*60)
    print("DIALOGUE PAPERS STATISTICS")
    print("="*60)
...
def main():
    ...
    output_dir = script_dir / 'dialogue_analysis_data'

    print(f"Aggregating dialogue papers from: {base_path}\n"
          f"(filtering to single-task = {TASK_NAME!r})\n")

    papers_data, metric_combinations, human_criteria_list, llm_judge_list = aggregate_dialogue_papers(base_path)
